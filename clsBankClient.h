...
private:
	...
	{
		return (Client.FirstName + Seperator + Client.LastName + Seperator + Client.Email + Seperator + Client.Phone + Seperator + Client.AccountNumber() + Seperator + Client.PinCode + Seperator + to_string(Client.AccountBalance));
	}
	...
	{
		return (clsDate::GetSystemDateString() + Seperator + to_string(Amount) + Seperator + AccountNumber() + Seperator + to_string(AccountBalance) + Seperator + DestinationClient.AccountNumber() + Seperator + to_string(DestinationClient.AccountBalance) + Seperator + UserName);
	}

	void _RegisterTransferLog(double Amount, clsBankClient& DestinationClient, string UserName)
	{
		string DataLine = _PrepareTransferLogLine(Amount, DestinationClient, UserName);
	...
	static stTransferLogRecord _ConvertTransferLogLineToRecord(string Line, string Seperator = "#//#")
	{
		...
		TransferLogRecord.Amount = stod(vTransferLogData[1]);
		TransferLogRecord.SourceAccountNumber = vTransferLogData[2];
		TransferLogRecord.SourceBalanceAfter = stod(vTransferLogData[3]);
		TransferLogRecord.DestinationAccountNumber = vTransferLogData[4];
		TransferLogRecord.DestinationBalanceAfter = stod(vTransferLogData[5]);
...
public:
	...
	static vector <stTransferLogRecord> GetTransferLogList()
	{
		...
		if (MyFile.is_open())
		{
			...
			while (getline(MyFile, Line))
			{
				vTransferLogRecord.emplace_back(_ConvertTransferLogLineToRecord(Line));
			}
			MyFile.close();
		}
